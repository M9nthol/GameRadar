...
void AMissleActor::Tick(float DeltaTime)
{
    ...
    if (CheckTargetCollision())
    {
        if (!bHasReachedTarget)
        {
            ...
            OnMissleReachedTarget.Broadcast();
        }
        Explode();
    }
    
    // Дополнительная проверка близости к цели
    if (IsNearTarget() && !bHasReachedTarget)
    {
        bHasReachedTarget = true;
        OnMissleReachedTarget.Broadcast();
    }
...
bool AMissleActor::IsNearTarget()
{
    FVector DistanceToTarget = GetActorLocation() - TargetPoint;
    return DistanceToTarget.Size() <= MinTargetDistance;
}
...
void AMissleActor::Explode()
{
    ...
    // Вызываем делегат перед уничтожением
    OnMissleDestroyed.Broadcast();
